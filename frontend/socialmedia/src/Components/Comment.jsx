      ...
      <section className="flex flex-wrap justify-center max-h-72 overflow-scroll ">
        ...
        {postcomments &&
          postcomments.reverse().map((comment) => {
            return <div className="shadow my-2  ">
              <div className="flex justify-start">

            <img src={comment.user.profile_picture} className="w-1/12 rounded-full object-cover"  />
            <p className="text-center font-bold">{comment.user.name}</p>
              </div>
            <p className="text-center mt-2 w-fit   p-2 pr-8 ">{comment.comment_text}</p>
          </div>
          })}
